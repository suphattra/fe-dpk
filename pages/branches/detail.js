...
export default function DetailBranch() {
    const breadcrumbs = [{ index: 1, href: '/branches', name: 'สาขาเเละแปลงงาน' }, { index: 2, href: '/branches', name: 'สร้างสาขาและแปลงงาน' }]
    ...
    const insertAddOnService = async () => {
        ...
        let lastElement = branchForm.length > 0 ? branchForm[branchForm.length - 1] : { index: 0 };
        let newService = {
            ...
            <LoadingOverlay active={loading} className="h-[calc(100vh-0rem)]" spinner text='Loading...'
                ...
                <div className="md:container md:mx-auto">
                    <form className="space-y-4" id='inputForm'>
                        {branchForm && branchForm.map((branch, index) => {
                            return (
                                <CardBranch index={index} branch={branch} onChange={onChange} deleteAddOnService={deleteAddOnService} onErrors={errors} />
                            )
                        })}
                    </form>
